...
static ui32 FK[4] = { 0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc };
static ui32 CK[32] = {
...
void SM4_Key_Gen(ui32* MK, ui32* rk){
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            tmp_8ptr[j] = SBox[tmp_8ptr[j]];
        }
        K[i + 4] = (K[i] ^ tmp) ^ (rotl(tmp, 13) ^ rotl(tmp, 23));
        rk[i] = K[i + 4];
    }
}
